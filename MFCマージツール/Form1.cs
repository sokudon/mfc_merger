...
namespace MFCマージツール
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (var k = 0; k < 4; k++)
            {
                if (Regex.IsMatch(mfc[k], "MFCU.csv"))
                {
                    mfcs[0] = mfc[k];
                }
                if (Regex.IsMatch(mfc[k], "MFCU_HG.csv"))
                {
                    mfcs[1] = mfc[k];
                }
                if (Regex.IsMatch(mfc[k], "MFCU_NEW.csv"))
                {
                    mfcs[2] = mfc[k];
                }
                if (Regex.IsMatch(mfc[k], "MFCU_STD.csv"))
                {
                    mfcs[3] = mfc[k];
                }
            }
            ...
            string mfc_hg = mfcs[1];
            string mfc_new = mfcs[2];
            string mfc_std = mfcs[3];
            string MFC = "";
            int count = 0;
            int counthg = 0;
            int countnew = 0;
            ...
            {
                string temp = lists[i];
                string tmp = lists[i].Substring(0,48);
